...
import 'package:rydr_app/models/publisher_insights_day.dart';
...
class PublisherInsightsMediaSummary {
  ...
  PublisherInsightsMediaSummary(
    ...
    items.forEach((PublisherInsightsMedia m) {
      flSpotsImpressions.add(FlSpot(idx.toDouble(),
          m.impressions == null ? 0.0 : m.impressions.toDouble()));

      flSpotsReach.add(
          FlSpot(idx.toDouble(), m.reach == null ? 0.0 : m.reach.toDouble()));

      flSpotsEngagements.add(FlSpot(
          idx.toDouble(),
          m.engagementRating == null
              ? 0.0
              : double.parse(m.engagementRating.toStringAsFixed(2))));
