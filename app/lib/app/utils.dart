...
DateTime maxDate = DateTime(2060, 12, 31, 0, 0, 0);

class Utils {
  static String replaceEncoding(String txt) =>
      txt.replaceAll('\\n', '\n').replaceAll('\\u0026', '\u0026');
  ...
  static String formatDateShortWithTime(DateTime dt) {
    final DateFormat f = dt.year == DateTime.now().year
  ...
  static void launchUrl(
    ...
    if (await canLaunch(url)) {
      AppAnalytics.instance.logScreen(
          trackingName != null ? 'external/$trackingName' : 'external/link');
      ...
      showSharedModalAlert(
        ...
        content: Text(unableMessage ?? "Unable to open the requested page"),
  ...
  static void goToProfile(
    ...
    /// determine if we can send the user to the full profile page, or if we
    /// should only open the instagram app to their profile
    ///
    /// 1. any soft-linked publisher we don't have a full profile for them
    /// 2. if we have a request that is not in a valid status where we support profile stats

    if (profile.isAccountSoft ||
        deal?.request?.canViewRequestersProfile == false) {
      showSharedModalAlert(
        context,
        Text("Open Instagram"),
        content: Text(
            "This will open ${profile.userName}'s profile in the Instagram app."),
        actions: <ModalAlertAction>[
          ModalAlertAction(
            label: "Cancel",
            onPressed: () {
      ...
      Navigator.of(context).pushNamed(
        AppRouting.getProfileRoute(profile.id),
        arguments: deal,
      );
    }
  }

  static void _launchAppleMaps(BuildContext context, Place place) async {
    launchUrl(context,
        'https://maps.apple.com/?daddr=${Uri.encodeFull(place.address.name)}&dirflg=d&t=r',
        trackingName: 'maps/apple');
  }

  static void _launchGoogleMaps(BuildContext context, Place place) async {
    launchUrl(context,
        'https://www.google.com/maps/search/?api=1&query=${Uri.encodeFull(place.address.name)}',
        trackingName: 'maps/google');
  }

  static void _launchWaze(BuildContext context, Place place) async {
    launchUrl(context,
        'https://www.waze.com/ul?ll=${place.address.latitude},${place.address.longitude}&navigate=yes',
        trackingName: 'maps/waze');
  }

  static void launchMapsAction(BuildContext context, Place place) async {
    if (appState.currentProfile.isCreator) {
      showSharedModalBottomActions(context,
          title: 'Get directions to ${place.name}',
          actions: <ModalBottomAction>[
            ModalBottomAction(
                icon: AppIcons.waze,
                child: Text("Use Waze"),
                onTap: () {
                  _launchWaze(context, place);
                }),
            ModalBottomAction(
                icon: AppIcons.google,
                child: Text("Use Google Maps"),
                onTap: () {
                  _launchGoogleMaps(context, place);
                }),
            ModalBottomAction(
                icon: AppIcons.apple,
                child: Text("Use Apple Maps"),
                onTap: () {
                  _launchAppleMaps(context, place);
                }),
            ModalBottomAction(
                icon: AppIcons.copy,
                child: Text("Copy Address to Clipboard"),
                onTap: () {
                  Clipboard.setData(ClipboardData(text: place.address.name));
