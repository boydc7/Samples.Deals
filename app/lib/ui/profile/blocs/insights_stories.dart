...
class InsightsStoriesBloc {
  final data = BehaviorSubject<InsightsStoriesBlocResponse>();
    ...
    if (_response.error == null && _response.models.isEmpty == false) {
      ...
      if (_response.models.length > 0) {
        ...
        if (_response.models.length > 10) {
          lastPostsIndex1 = _response.models.length >= 50
              ? "Last 50"
              : "Last ${_response.models.length}";

          // we take one more than we need if available, but show only 5. this allows for the fading of the chart on the left
          _media1 = _response.models.length > 50
              ? _mediaSorted.skip(_response.models.length - 51).toList()
              : _mediaSorted.take(_response.models.length).toList();
        }

        if (_response.models.length > 50) {
          lastPostsIndex2 = _response.models.length >= 100
              ? "Last 100"
              : "Last ${_response.models.length}";

          // we take one more than we need if available, but show only 5. this allows for the fading of the chart on the left
          _media2 = _response.models.length > 100
              ? _mediaSorted.skip(_response.models.length - 101).toList()
              : _mediaSorted.take(_response.models.length).toList();
        }
      }
      toggleShowIndex(0);
...
class InsightsStoriesBlocResponse {
  DioError error;
