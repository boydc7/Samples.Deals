...
public class OrmLiteDataService : BaseDataService, IRydrDataService
{
    ...
    ///     the batch.
    /// </summary>
    public async Task UpdateOnlyAsync<T, TIdType>(T partialModel, Func<IDbConnection, SqlExpression<T>> selectFilter,
            ...
            try
            {
                                                              ...
                                                              .ToLazyBatchesOf(250))
                {
                    await _storageMarshaller.StoreRangeAsync(updateBatch,
                                                             m => dbConnection.UpdateOnlyAsync(() => partialModel, updateFilter(m)),
                                                             idResolver);
                }
            }
            catch(Exception x) when(OnDataAccessFail(x, dbConnection.GetLastSql())) { }
