    ...
    public class DynamoDealRequestService : IDealRequestService
    {
                                                                                      ...
                                                                                      DateTime statusChangeStart, DateTime statusChangeEnd, long inWorkspaceId = 0)
        {
            ...
            return _dynamoDb.GetItemsFromAsync<DynDealRequest, DynItemTypeOwnerSpaceReferenceGlobalIndex>(query.Select(i => new
                                                                                                               ...
                                                                                                               .ExecAsync(),
                                                                                                          idSelector: i => new DynamoId(i.Id,
        ...
        public async IAsyncEnumerable<DealResponse> GetDealResponseRequestExtendedAsync(ICollection<DynDealRequest> dynDealRequests)
        {
            ...
            var totalApprovedMap = await _dynamoDb.GetItemsAsync<DynDealStat>(deals.Select(d => new DynamoId(d.DealId,
                                                                                                             DynDealStat.BuildEdgeId(d.PublisherAccountId,
                                                                                                                                     DealStatType.TotalApproved))))
                                                  .ToDictionarySafe(ds => ds.DealId);

            var completionMediaMap = await _dynamoDb.GetItemsAsync<DynPublisherMedia>(dynDealRequests.Where(dr => !dr.CompletionMediaIds.IsNullOrEmptyRydr())
            ...
            var requestStatusChangeMap = await _dynamoDb.GetItemsAsync<DynDealRequestStatusChange>(dynDealRequests.SelectMany(dr => _allDealRequestStatusStrings.Select(drs => new DynamoId(dr.DealId,
                                                                                                                                                                                            DynDealRequestStatusChange.BuildEdgeId(drs,
                                                                                                                                                                                                                                   dr.PublisherAccountId)))))
                                                        .ToDictionaryManySafe(sc => (sc.DealId, sc.PublisherAccountId));

            var lifetimeStatsMap = await _dynamoDb.GetItemsAsync<DynPublisherMediaStat>(dynDealRequests.Where(dr => !dr.CompletionMediaIds.IsNullOrEmptyRydr())
            ...
            var lastMessageMap = await _dynamoDb.GetItemsFromAsync<DynDialogMessage, DynItemMap>(_dynamoDb.GetItemsAsync<DynItemMap>(dialogKeyMap.Select(k => new DynamoId(k.Key.DealId,
        ...
        public async Task<List<DynDealRequest>> GetDealRequestsAsync(IEnumerable<long> forDealIds, long publisherAccountId)
        {
            ...
            var results = await _dynamoDb.GetItemsAsync<DynDealRequest>(forDealIds.Select(i => new DynamoId(i, publisherAccountEdgeId)))
                                         .Take(5000)
                                         .ToList();
        ...
        public Task<List<DynDealRequest>> GetDealRequestsAsync(long dealId, IEnumerable<long> publisherAccountIds)
            => _dynamoDb.GetItemsAsync<DynDealRequest>(publisherAccountIds.Select(i => new DynamoId(dealId, i.ToEdgeId())))
                        .Take(5000)
                        .ToList();
        ...
        public IAsyncEnumerable<DynDealRequest> GetPublisherAccountRequestsAsync(long publisherAccountId, IEnumerable<DealRequestStatus> statuses = null)
        {
            ...
            return _dynamoDb.GetItemsFromAsync<DynDealRequest, DynItemEdgeIdGlobalIndex>(query.ExecAsync(),
                                                                                         i => i.GetDynamoId(),
                                                                                         take: 10000);
        }
        ...
        {
            var existingDealRequest = await _dynamoDb.GetItemAsync<DynDealRequest>(dealId, publisherAccountId.ToEdgeId(), true);
        ...
        {
            if (!(await DoChangeRequestStatusAsync(dynDealRequest, DealRequestStatus.Delinquent, r => CanBeDelinquentAsync(r, ignoreTimeConstraint: true))))
            {
        ...
        private async Task<DealRequestExtended> GetDealRequestExtendedAsync(DynDealRequest dealRequest)
        {
            var completionMedia = dealRequest.CompletionMediaIds.IsNullOrEmpty()
                                      ? null
                                      : await _dynamoDb.GetItemsAsync<DynPublisherMedia>(dealRequest.CompletionMediaIds
                                                                                                    .Select(mi => new DynamoId(dealRequest.PublisherAccountId, mi.ToEdgeId())))
                                                       .Where(dm => dm != null && !dm.IsDeleted())
                                                       .Take(500)
                                                       .ToList();

            var requestStatusChanges = await _dynamoDb.GetItemsAsync<DynDealRequestStatusChange>(_allDealRequestStatusStrings.Select(drs => new DynamoId(dealRequest.DealId,
                                                                                                                                                         DynDealRequestStatusChange.BuildEdgeId(drs,
            ...
            var lifetimeStats = completionMedia.IsNullOrEmpty()
                                    ? null
                                    : await _dynamoDb.GetItemsAsync<DynPublisherMediaStat>(completionMedia.Select(cm => new DynamoId(cm.PublisherMediaId,
                                                                                                                                     DynPublisherMediaStat.BuildEdgeId(FbIgInsights.LifetimePeriod,
                                                                                                                                                                       FbIgInsights.LifetimeEndTime))))
