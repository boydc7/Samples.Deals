...
public class CustomSubscriptionPlanService : ISubscriptionPlanService
{
    ...
    private readonly Dictionary<string, long> _reversedProductIdMap = new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, DynItemMap> _priceIdToMap = new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, DynItemMap> _productPriceInCentsToMap = new();
    ...
    public static string CustomPerPostProductId { get; } = RydrEnvironment.GetAppSetting("Stripe.Products.Custom.Post", "prod_HZeU0UXqPstshF");
    ...
    public async ValueTask<IEnumerable<string>> GetPlanIdsForSubscriptionTypeAsync(SubscriptionType subscriptionType, int customMonthlyFeeCents = 0, int customPerPostFeeCents = 0)
    {
        ...
        var monthlyPlanPrice = subscriptionType switch
                               {
                                   SubscriptionType.ManagedCustomAdvance => await GetOrCreateMapForCustomPriceAsync(_stripeCustomAdvanceProductId, customMonthlyFeeCents, false),
                                   SubscriptionType.ManagedCustom => await GetOrCreateMapForCustomPriceAsync(_stripeCustomArrearsProductId, customMonthlyFeeCents, false),
                                   _ => throw new ArgumentOutOfRangeException()
                               };

        var perPostPlanPrice = await GetOrCreateMapForCustomPriceAsync(CustomPerPostProductId, customPerPostFeeCents, true);
                                                                          ...
                                                                          int customMonthlyFeeCents = 0, int customPerPostFeeCents = 0)
    {
        ...
        switch (usageType)
        {
            ...
            {
                var perPostMap = await GetOrCreateMapForCustomPriceAsync(CustomPerPostProductId, customPerPostFeeCents, true);
            ...
            case SubscriptionUsageType.SubscriptionFee:
            {
                switch (subscriptionType)
                {
                    ...
                    {
                        var advanceMap = await GetOrCreateMapForCustomPriceAsync(_stripeCustomAdvanceProductId, customMonthlyFeeCents, false);

                        return advanceMap.MappedItemEdgeId;
                    }
                    case SubscriptionType.ManagedCustom:
                    {
                        var arrearsMap = await GetOrCreateMapForCustomPriceAsync(_stripeCustomArrearsProductId, customMonthlyFeeCents, false);
    ...
    private async ValueTask<DynItemMap> GetOrCreateMapForCustomPriceAsync(string productId, int priceInCents, bool isPerUnitUsage)
    {
        ...
        if (stripePrice == null)
        {
            ...
            else if (productId.EqualsOrdinalCi(CustomPerPostProductId) ||
                     productId.EqualsOrdinalCi(_stripeCustomArrearsProductId))
            {
                stripePrice = await stripe.CreateProductMeteredPriceAsync(productId, priceInCents, isPerUnitUsage, lookupKey);
            }
        }
