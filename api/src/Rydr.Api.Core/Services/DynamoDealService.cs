...
public class DynamoDealService : IDealService
{
    ...
    public Task<IReadOnlyList<DynDeal>> GetDynDealsAsync(IEnumerable<DynamoId> dealIds)
        => _dynamoDb.QueryItemsAsync<DynDeal>(dealIds)
                    .Take(1000)
                    .ToListReadOnly();
    ...
    public async Task<List<DynDealStat>> GetDealStatsAsync(long dealId)
    {
        ...
        var results = await _dynamoDb.QueryItemsAsync<DynDealStat>(_allDealStatTypeString.Select(dst => new DynamoId(dealId,
        ...
        where T : IHasDealId, IHasPublisherAccountId
    {
        ...
        await foreach (var dynDealStat in _dynamoDb.QueryItemsAsync<DynDealStat>(deals.SelectMany(d => _allDealStatTypeString.Select(dst => new DynamoId(d.DealId,
...
SET ModifiedBy = :cid,
        ...
        Dictionary<long, Hashtag> HashtagMap, Dictionary<long, PublisherAccount> PublisherMap)> GetDealMapsForTransformAsync(IReadOnlyCollection<DynDeal> dynDeals)
    {
        ...
        var publisherMediaMap = await _dynamoDb.QueryItemsAsync<DynPublisherMedia>(dynDeals.Where(r => !r.PublisherMediaIds.IsNullOrEmpty())
        ...
        var placeMap = distinctPlaces.Count > 0
                           ? await _dynamoDb.QueryItemsAsync<DynPlace>(distinctPlaces.Select(h => h.ToItemDynamoId()))
