...
public class TimestampedMapItemService : TimestampCachedServiceBase<DynItemMap>, IMapItemService
{
    ...
    public DynItemMap TryGetMap(long id, string edgeId)
    {
        ...
        return GetModel(string.Concat(id, "|", edgeId),
                        () => _dynamoDb.GetItem<DynItemMap>(id, edgeId));
    }

    public Task<DynItemMap> TryGetMapAsync(long id, string edgeId)
    {
        if (edgeId.IsNullOrEmpty())
        {
        ...
        return GetModelAsync(string.Concat(id, "|", edgeId),
                             () => _dynamoDb.GetItemAsync<DynItemMap>(id, edgeId));
    }
    ...
    public async Task<DynItemMap> GetMapAsync(long id, string edgeId)
    {
        var map = await TryGetMapAsync(id, edgeId);
    ...
    public DynItemMap TryGetMapByHashedEdge(DynItemType mapItemType, string hashedEdgeValue)
    {
        var longId = hashedEdgeValue.ToLongHashCode();

        return TryGetMap(longId, DynItemMap.BuildEdgeId(mapItemType, hashedEdgeValue));
    }

    public Task<DynItemMap> TryGetMapByHashedEdgeAsync(DynItemType mapItemType, string hashedEdgeValue)
    {
        var longId = hashedEdgeValue.ToLongHashCode();

        return TryGetMapAsync(longId, DynItemMap.BuildEdgeId(mapItemType, hashedEdgeValue));
    }
