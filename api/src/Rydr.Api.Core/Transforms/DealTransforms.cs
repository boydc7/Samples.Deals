...
namespace Rydr.Api.Core.Transforms
{
    public static class DealTransforms
    {
        ...
        public static async Task<EsDeal> ToEsDealAsync(this DynDeal source)
        {
                ...
                foreach (var restriction in source.Restrictions)
                {
                    switch (restriction.Type)
                    {
                        ...
                        case DealRestrictionType.MinFollowerCount:
                            minFollowerCount = restriction.Value.ToLong().MinGz(0);
            ...
            {
                var invitedDealRequestIds = await _dynamoDb.GetItemsAsync<DynDealRequest>(source.InvitedPublisherAccountIds
        ...
        {
            var statusUpdatedOn = source.ReferenceId.ToLong().ToDateTime();
        ...
        public static void PopulateApprovalsRemaining(this DealResponse dealResponse)
        {
            ...
            var totalApproved = dealResponse.Stats.FirstOrDefault(s => s.Type == DealStatType.TotalApproved)?.Value.ToLong() ?? 0;
        ...
        private static async Task DecorateAsyncDealAsync(Deal toDecorate, DynDeal decorateWith)
        {
            ...
            {
                toDecorate.PublisherMedias = await _dynamoDb.GetItemsAsync<DynPublisherMedia>(decorateWith.PublisherMediaIds
...
#pragma warning restore 618
            ...
            to.StatusLastChangedOn = source.ReferenceId.ToLong();
