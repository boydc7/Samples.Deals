...
public static class WorkspaceExtensions
{
    ...
    public static SubscriptionType ToSubscriptionType(this Subscription source, SubscriptionItem managedPlanItem, SubscriptionType defaultType = SubscriptionType.None)
    {
        if (source == null || (managedPlanItem?.Quantity).GetValueOrDefault() <= 0)
        {
    ...
    public static DynWorkspaceSubscription ToDynWorkspaceSubscription(this Subscription subscription, SubscriptionItem managedPlanItem,
                                                                      long workspaceId, DynWorkspaceSubscription existing = null,
                                                                      SubscriptionType subscriptionType = SubscriptionType.None)
    {
        ...
        to.SubscriptionType = subscriptionType == SubscriptionType.None
                                  ? ToSubscriptionType(subscription, managedPlanItem, existing?.SubscriptionType ?? SubscriptionType.None)
                                  : subscriptionType;

        to.Quantity = (managedPlanItem?.Quantity ?? existing?.Quantity).GetValueOrDefault();
        to.UnitPriceCents = (managedPlanItem?.Plan.Amount).Gz(existing?.UnitPriceCents);
        to.Interval = (managedPlanItem?.Plan.Interval).Coalesce(existing?.Interval);
        to.IntervalCount = (managedPlanItem?.Plan.IntervalCount).Gz(existing?.IntervalCount ?? 0);
        to.ProductId = (managedPlanItem?.Plan.ProductId).Coalesce(existing?.ProductId);
        to.PlanId = (managedPlanItem?.Plan.Id).Coalesce(existing?.PlanId);
        ...
        to.SubscriptionStartedOn = (subscription.StartDate > DateTimeHelper.MinApplicationDate
                                        ...
                                        : subscription.BillingCycleAnchor).ToUnixTimestamp().Gz(existing?.SubscriptionStartedOn ?? 0);
        ...
        to.BillingCycleAnchor = subscription.BillingCycleAnchor.ToUnixTimestamp();
