...
namespace Rydr.Api.Core.Models.Internal
{
    public class CacheConfig
    {
        ...
        private static readonly ConcurrentDictionary<string, CacheConfig> _configMap = new ConcurrentDictionary<string, CacheConfig>();

        private static readonly CacheConfig _nullConfig = null;

        private static readonly CacheConfig _defaultConfig = new CacheConfig
                                                             {
                                                                 DurationSeconds = 60 * 20
                                                             };

        public static CacheConfig FrameworkConfig { get; } = new CacheConfig
                                                             {
                                                                 DurationSeconds = 60 * 60 * 2
                                                             };

        public static CacheConfig LongConfig { get; } = new CacheConfig
                                                        {
                                                            DurationSeconds = 60 * 60 * 24 * 15
                                                        };

        public static CacheConfig FromSeconds(int seconds) => new CacheConfig
                                                              {
                                                                  DurationSeconds = seconds
                                                              };

        public static CacheConfig FromMinutes(int minutes) => new CacheConfig
                                                              {
                                                                  DurationSeconds = minutes * 60
                                                              };

        public static CacheConfig FromHours(int hours) => new CacheConfig
                                                          {
                                                              DurationSeconds = hours * 60 * 60
                                                          };

        public static CacheConfig FromDays(int days) => new CacheConfig
                                                        {
