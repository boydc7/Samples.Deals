    ...
    public class CheckoutServiceInteral : BaseInternalOnlyApiService
    {
        ...
        public async Task Post(CheckoutSubscriptionCreated request)
        {
            ...
            if (subscriptionType.IsManagedSubscriptionType())
            {
                ...
                var dynManagingWorkspace = await _workspaceService.TryGetWorkspaceAsync(managingWorkspaceId.ToLong());
                var dynManagedPublisherAccount = await _publisherAccountService.TryGetPublisherAccountAsync(managedPublisherAccountId.ToLong());
                ...
                if ((existingPublisherSubscription?.StripeSubscriptionId).EqualsOrdinalCi(stripeSubscription.Id, matchIfBothEmpty: false))
                {
                ...
                else
                {
                    ...
                    var subscriptionCreated = await _workspacePublisherSubscriptionService.AddManagedSubscriptionAsync(dynManagingWorkspace.Id, dynManagedPublisherAccount, subscriptionType,
                                                                                                                       ...
                                                                                                                       backdateTo: subscriptionStartedOnString.ToDateNullable(),
...
",
        ...
        {
            var workspaceId = request.ClientReferenceId.ToLong();
            ...
            else
            {
                ...
                dynWorkspaceSubscription = await ProcessNewPayBerBusinessSubscriptionAsync(pubAccountIds.Select(s => s.ToLong()).AsListReadOnly(),
...
",
                                                                                               ...
                                                                                               DynWorkspace workspace, DynWorkspaceSubscription existingWorkspaceSubscription)
        {
            ...
            var subscriptionType = workspace.WorkspaceType == WorkspaceType.Admin
                                       ...
                                       : stripeSubscription.ToSubscriptionType();

            var dynWorkspaceSubscription = stripeSubscription.ToDynWorkspaceSubscription(workspace.Id, existingWorkspaceSubscription, subscriptionType);
        ...
        private async Task ProcessPayPerBusinessSubscriptionUpdate(DynWorkspaceSubscription existingWorkspaceSubscription)
        {
            ...
            if (stripeSubscription.Quantity.Gz(0) == existingWorkspaceSubscription.Quantity)
            { // Update what we have for subscription attributes and we're all done
                var workspaceSubscription = stripeSubscription.ToDynWorkspaceSubscription(existingWorkspaceSubscription.SubscriptionWorkspaceId, existingWorkspaceSubscription);
        ...
        private async Task OnCustomerUpsert(string stripeCustomerId)
        {
            ...
            var workspaceId = stripeCustomer.Metadata.GetValueOrDefault("WorkspaceId").ToLong();
            var managedPublisherAccountId = stripeCustomer.Metadata.GetValueOrDefault("PublisherAccountId").ToLong();
                                          ...
                                          string stripeInvoiceId = null, StripeService stripe = null)
        {
            ...
            var stripeInvoice = stripeInvoiceId.IsNullOrEmpty()
                                    ? await stripe.GetUpcomingInvoiceAsync(subscriptionId: stripeSubscription.Id)
