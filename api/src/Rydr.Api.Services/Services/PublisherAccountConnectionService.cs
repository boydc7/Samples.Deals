    ...
    public class PublisherAccountConnectionService : BaseAuthenticatedApiService
    {
        ...
        public async Task<QueryResponse<PublisherAccountInfo>> Get(QueryPublisherAccountConnections request)
        {
            ...
            if (request.ToPublisherAccountId.HasValue)
            {
                ...
                var publisherAuthorization = await _dynamoDb.GetItemsAsync<DynAuthorization>(authTypesToGet.Select(p => new DynamoId(request.Id, string.Concat(p, request.ToPublisherAccountId.Value))))
                                                            .FirstOrDefaultAsync(da => (!request.LastConnectedAfter.HasValue || da.ReferenceId.ToLong() > request.LastConnectedAfter.ToUnixTimestamp()) &&
                                                                                       (!request.LastConnectedBefore.HasValue || da.ReferenceId.ToLong() < request.LastConnectedBefore.ToUnixTimestamp()));
