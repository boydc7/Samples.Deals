...
namespace Rydr.Api.Services.Services
{
    public class PublisherMediaAnalysisService : BaseApiService
    {
        ...
        private async Task<IReadOnlyList<PublisherMedia>> DoGetPublisherAccountMediaSearch(PublisherAccountMediaVisionSectionSearchDescriptor searchDescriptor)
        {
            ...
            {
                var publisherMedias = await _dynamoDb.GetItemsAsync<DynPublisherMedia>(searchResults.Select(t => new DynamoId(searchDescriptor.PublisherAccountId, t.ToEdgeId())))
            ...
            var randomPublisherMedias = await _dynamoDb.GetItemsAsync<DynPublisherMedia>(searchMaterialized.Select(t => new DynamoId(searchDescriptor.PublisherAccountId, t.ToEdgeId())))
