...
public class PublisherMediaService : BaseAuthenticatedApiService
{
    ...
    public async Task<OnlyResultsResponse<PublisherApprovedMedia>> Get(GetPublisherApprovedMedias request)
    {
        ...
        if (request.DealId > 0)
        {
            ...
            dynApprovedMedias = await _dynamoDb.QueryItemsAsync<DynPublisherApprovedMedia>(dynDeal.PublisherApprovedMediaIds.Select(ami => new DynamoId(dynDeal.PublisherAccountId, ami.ToEdgeId())))
                                               .ToList();
        }
        else
        {
    ...
    public async Task<OnlyResultsResponse<PublisherMedia>> Get(GetRecentMedia request)
    {
        ...
        var publisherAccount = await _publisherAccountService.GetPublisherAccountAsync(request.PublisherIdentifier.EqualsOrdinalCi("me")
                                                                                           ...
                                                                                           : request.PublisherIdentifier.ToLong(0));
        ...
        { // Go get an medias we already have based on the info we got from the publisher...
            await foreach (var dynRecentPublisherMedia in _dynamoDb.GetItemsFromAsync<DynPublisherMedia, DynItemMap>(_dynamoDb.QueryItemsAsync<DynItemMap>(recentPublisherMedia.Select(f => new DynamoId(publisherAccount.PublisherAccountId,
    ...
    public async Task<OnlyResultsResponse<PublisherMedia>> Get(GetRecentPublisherMedia request)
    {
        ...
        // Go get an medias we already have based on the info we got from the publisher...
        var existingMediaMap = await _dynamoDb.QueryItemsAsync<DynItemMap>(recentPublisherMedia.Select(f => new DynamoId(publisherAccount.PublisherAccountId,
