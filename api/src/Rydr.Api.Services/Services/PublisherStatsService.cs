...
public class PublisherStatsService : BaseApiService
{
    ...
    private static readonly HashSet<string> _suggestiveImageLabels = new(StringComparer.OrdinalIgnoreCase)
                                                                     {
                                                                         "suggestive",
                                                                         "revealing",
                                                                         "nudity",
    ...
    public async Task<OnlyResultsResponse<ContentTypeStat>> Get(GetPublisherContentStats request)
    {
        ...
        var dynStats = await _dynamoDb.QueryItemsAsync<DynPublisherMediaStat>(dynMedia.Select(l => new DynamoId(l.Key, DynPublisherMediaStat.BuildEdgeId(FbIgInsights.LifetimePeriod,
    ...
    public async Task<OnlyResultsResponse<AudienceGrowthResult>> Get(GetPublisherAudienceGrowth request)
    {
        ...
        var dailyStats = await _dynamoDb.QueryItemsAsync<DynDailyStat>(dailyKeys)
