            ...
            .MustAsync(async (ri, t) =>
                       {
                           ...
                           var dealRequest = await DealExtensions.DefaultDealRequestService
                                                                 .GetDealRequestAsync(dealMap.ReferenceNumber.Value, dealMap.MappedItemEdgeId.ToLong(0));
            ...
            .MustAsync(async (r, newStatus, ctx, t) =>
                       {
                           ...
                           var existingDealRequest = await ValidationExtensions._dynamoDb.GetItemAsync<DynDealRequest>(r.DealId, requestPublisherId.ToEdgeId(), t);
                           ...
                           var result = newStatus switch
                                        {
                                            ...
                                            DealRequestStatus.Delinquent when !(await dealRequestService.CanBeDelinquentAsync(existingDealRequest, true)) => false,
                                            _ => true
                                        };
